...
public class AI {
    ...
    public void follow(int referenceID, int targetID){
        ...
        double [] distance = findDistanceVector(referenceID, targetID);
        double deltaX = distance[0];
        double deltaY = distance[1];
        double magnitude = Math.pow(Math.pow(deltaX, 2) + Math.pow(deltaY, 2), .5);
        if (LOSComp == null || isInLOS(targetID, referenceID, magnitude, LOSComp.getLOS())) {
            myEntityManager.moveInDirection(referenceID, findDirection(distance));
        }
    }
